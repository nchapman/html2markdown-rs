...
fn inline_code_newline_replaced_with_space() {
    ...
    use html_to_markdown::mdast::*;
    use html_to_markdown::StringifyOptions;
    ...
    let md = html_to_markdown::mdast_to_string(&node, &StringifyOptions::default());
...
fn ordered_list_marker_escaped_dot() {
    let md = html_to_markdown::convert("<p>1. foo</p>");
    assert_eq!(md, "1\\. foo\n");
}
...
fn ordered_list_marker_multi_digit() {
    let md = html_to_markdown::convert("<p>10. foo</p>");
    assert_eq!(md, "10\\. foo\n");
}
...
fn indented_code_when_fences_false() {
    ...
    let md = html_to_markdown::mdast_to_string(&node, &opts);
    assert_eq!(md, "    hello\n    world\n");
}
...
fn list_item_indent_tab() {
    use html_to_markdown::{convert_with, ListItemIndent, Options};
...
fn less_than_before_tag_escaped_at_break() {
    let md = html_to_markdown::convert("<p>&lt;div&gt; text</p>");
    assert!(
        md.starts_with("\\<div>"),
